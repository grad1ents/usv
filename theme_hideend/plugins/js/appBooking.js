...
Vue.component('hasil-pspbmn', {
            ...
            props: ['databook','tipebook','jenisform'],
            data() {
                 return {
                    url: myUrl,
                    chooseBook:this.databook,
                    verifikasi:{
                        id: '',
                        idPengajuan : '',
            ...
            methods:{
                ...
                finishVerifikasiSendEmail() {
                    let namaSurat = this.databook.kementerian_lembaga+'-'+this.databook.noSurat_pemohon
                    let tujuanEmail = this.databook.email_petugas
                ...
                getDataVerifikasiDokumen:function(){
                    let self = this
                    axios.post(this.url + "/hideend/verifikasi/checkDocumentVerifikasi/"+this.chooseBook.id).then(function(response) {
                ...
                addVerifikasi:function(status) {
                    ...
                    this.verifikasi.status_book = status
                ...
                updateStatusPengajuan(status){
                    let book= {
                                idPengajuan:this.chooseBook.id,
                                status_book:status
                              }
                    var formData = this.formData(book);
                    axios.post(this.url + "/hideend/book/updateStatusPengajuan", formData).then(function(response) {
                            if (!response.data.error) {
                                return true;
                            } else {
                                return false;
...
Vue.component('table-verifikasi-pspbmn', {
            ...
            methods:{
                    ...
                    showAll(){ 
                        let self = this
                        axios.post(this.url+"/hideend/book/showAll").then(function(response){
                                 if(response.data.book == null){
                                        self.noResult()
                                    }else{
                                        self.getData(response.data.book);
                                    }
                        })
                    },
                    getData(book){
                        this.emptyResult = false; // become false if has a record
                        this.totalData = book.length //get total of user
                        this.book = book.slice(this.currentPage * this.rowCountPage, (this.currentPage * this.rowCountPage) + this.rowCountPage); //slice the result for pagination
                        
                         // if the record is empty, go back a page
                        if(this.book.length == 0 && this.currentPage > 0){ 
                    ...
                    getDataChoosePengajuan(){
                         this.$emit('send-data', this.chooseBook)
                    },
                    getJenisForm(){
...
var v = new Vue({
    ...
    computed: {
        ...
        hrefFileSuratPermohon: function () {
            // `this` points to the vm instance
            return this.url+'/uploads/'+this.chooseBook.fileSuratPermohon
        },
        // a computed getter
        hrefFileDaftarRincian: function () {
            // `this` points to the vm instance
            return this.url+'/uploads/'+this.chooseBook.fileDaftarRincian
        },
        // a computed getter
        hrefFileDokumenKelengkapan: function () {
            // `this` points to the vm instance
            return this.url+'/uploads/'+this.chooseBook.fileDokumenKelengkapan
        }
    },
    methods: {
